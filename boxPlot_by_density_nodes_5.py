...
# first boxplot pair density 25%
bp = boxplot(listofalltimes[0], positions = [1, 2, 3],widths = 0.6,patch_artist = True)
boxsettings(bp)

# second boxplot pair density 30%
bp = boxplot(listofalltimes[1], positions = [6, 7, 8], widths = 0.6,patch_artist = True,)
boxsettings(bp)

# thrid boxplot pair density 35%
bp = boxplot(listofalltimes[2], positions = [11,12,13], widths = 0.6,patch_artist = True)
boxsettings(bp)

#fourth boxplot pair density 40%
bp = boxplot(listofalltimes[3], positions = [16,17,18], widths = 0.6,patch_artist = True)
boxsettings(bp)

#fifth boxplot pair density 45%
bp = boxplot(listofalltimes[4], positions = [21,22,23], widths = 0.6,patch_artist = True)
boxsettings(bp)

#sixth boxplot pair density 50%
bp = boxplot(listofalltimes[5], positions = [26,27,28], widths = 0.6,patch_artist = True)
boxsettings(bp)
...
xlim(0,30)
ax.set_xticklabels(['25%', '30%', '35%','40%','45%','50%'])
...
plt.legend(handles=[u2,u3,u4],numpoints = 1,loc = 4)


plt.xlabel('densities')
plt.ylabel('computation time (minutes)')
plt.title('n = 5, number of graphs per u = 100' ,multialignment='center')

plt.savefig('bp_nodes_5.svgz')
plt.show()
